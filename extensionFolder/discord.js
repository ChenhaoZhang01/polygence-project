...
function start() {
  chrome.storage.local.get(['discordSetting'], function(result) {
      val= result.discordSetting;
      choice(val);
  });
}
start()
    ...
    if (val === 'def') {
        ...
        function handleKeyPress(event) {
          if (event.key === "Enter" && !event.shiftKey) {
            tweetTextarea = document.querySelector(`span[data-slate-string="true"]`);
            if (globalData == 1) {
        ...
        setInterval(() => {
          const tweetTextarea = document.querySelector(`span[data-slate-string="true"]`);
          if (globalglobalData == 3) {
              tweetTextarea.style.color = '8B0000';
          } else if (globalglobalData == 2) {
              tweetTextarea.style.color = 'red';
          } else if (globalglobalData == 1) {
              tweetTextarea.style.color = 'red';
          } else if (globalglobalData == 0) {
              tweetTextarea.style.color = 'white';
          } else if (globalglobalData == 'none') {
              tweetTextarea.style.color = 'white';
          }
      },200);
        //keydown code
        ...
        function trackMessage1() {
          const tweetTextarea1 = document.querySelector(`span[data-slate-string="true"]`);
          if (tweetTextarea1) {
        ...
        function handleKeyPress2(event) {
          if (event.key === "Enter" && !event.shiftKey) {
            tweetTextarea2 = document.querySelector(`span[data-slate-string="true"]`);
            if (globalData == 1) {
                console.log("detecting microaggression...");
        ...
        function addInputFieldListener2() {
          const inputField2 = document.querySelector('div[role="textbox"]');
          if (inputField2) {
        ...
        function trackMessage2() {
          const tweetTextarea2 = document.querySelector(`span[data-slate-string="true"]`);
          if (tweetTextarea2) {
